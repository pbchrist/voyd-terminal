...
- FRONTIER ADVANCE IS MANDATORY. Every accepted cycle must extend at least one active frontier listed in `story_room/frontier.json` with a NEW scene file under `story/scenes/` that is reachable by a choice link from its parent scene. Deepening, restructuring, or repairing existing scenes is legitimate work but is NEVER sufficient on its own: a cycle that adds no new reachable scene has not advanced the story and must keep working until one exists.
...
- Prefer extending the frontier that has gone longest without extension, so depth accumulates along a path toward an ending instead of every path widening at the same rate forever.
